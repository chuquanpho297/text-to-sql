...
"""
Continue Fine-tuning Script for A100 GPU - XiYanSQL-QwenCoder-32B-2412

This script continues fine-tuning from a previously trained A100 model:
- Base model: quanminhnguyen0508/XiYanSQL-QwenCoder-32B-2412-100kSQL_finetuned-A100_79GB
...
# =============================================================================
# CONTINUE FROM EXISTING A100 MODEL
model_name = "quanminhnguyen0508/XiYanSQL-QwenCoder-32B-2412-100kSQL_finetuned-A100_79GB"
max_seq_length = 1024  # Keep same as original training
dtype = None
load_in_4bit = True  # Keep 4-bit quantization for consistency

# Fixed GPU configuration for A100
gpu_config = {
    "gpu_count": 1,
    "gpu_types": ["A100-80GB"],
    "total_memory": 80.0,
    "setup_type": "Single-A100-80GB",
...
if torch.cuda.is_available():
    ...
    if "A100" not in gpu_name:
        print(f"⚠️ Warning: Expected A100, found {gpu_name}")
else:
    raise ValueError("CUDA not available. A100 GPU required for this script.")
...
wandb_run_name = f"xiyanSQL-32b-continue-A100-{max_seq_length}seq-200steps"

tags = ["sql", "text2sql", "32b", "lora", "unsloth", "continue-training", "a100"]

# A100 optimized settings
per_device_batch_size = 8
gradient_accumulation = 6
print("🚀 A100 80GB - Optimized for stability (continue training)")
...
wandb.init(
    ...
    config={
        # Model configuration
        "base_model": "quanminhnguyen0508/XiYanSQL-QwenCoder-32B-2412-100kSQL_finetuned-A100_79GB",
        ...
        "hardware_optimization": "A100_continue_optimized",
    },
    tags=tags,
    notes=f"Continue fine-tuning from A100 model: {model_name}",
)
...
print("This model was previously fine-tuned on A100, continuing training...")
...
new_model_name = f"{hf_username}/XiYanSQL-QwenCoder-32B-2412-100kSQL_continued-A100_79GB"
...
print("- Continue from: A100 fine-tuned model")
...
instruct_finetune_args = UnslothTrainingArguments(
    output_dir="/tmp/continue_finetune_a100",
    seed=3407,
    logging_steps=2,
    save_steps=20,
    save_strategy="steps",
